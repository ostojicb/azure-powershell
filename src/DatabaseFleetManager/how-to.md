# How-To
This document describes how to develop for `Az.DatabaseFleetManager`.

## Building `Az.DatabaseFleetManager`
...
## Testing `Az.DatabaseFleetManager`
...
## Packing `Az.DatabaseFleetManager`
...
There are multiple scripts created for performing different actions for developing `Az.DatabaseFleetManager`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DatabaseFleetManager.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DatabaseFleetManager.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DatabaseFleetManager`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DatabaseFleetManager` into the session.
